...
def get_queue_next_token(delta_text_queue):
    timeout = int(os.getenv("OpenVINO_FASTAPI_TIMEOUT", 60))
...
async def chat_stream_generator(local_model, delta_text_queue, request_id):
    ...
    while True:
        ...
        if not hasattr(delta_text_queue, 'empty'):
            ...
            try:
                token_output += delta_text
            except:
                print("!!! token_output += delta_text can only concatenate str (not NoneType) to str")
            #print("delta_text, remain",delta_text, remain)
...
def set_parameters(req):
    if req.max_tokens is None:
        n_predict = 2048  # Requested value for the demo
        # n_predict = 128  # Default value of OpenWebui
        # n_predict = local_model.model.config.max_position_embeddings
    else:
    ...
    # else:
    #     do_sample = False
    do_sample = False  # Not supported for now
...
async def process_requests(local_model, result_dict):
    while True:
        await asyncio.sleep(0)
        await local_model.process_step(result_dict, processor)
